...
// ================================================================================================
// ابدأ: محتوى من ملف "صور المجله.txt" (يعتمد على jQuery)
// ================================================================================================
if (typeof jQuery !== 'undefined') {
    jQuery(function($) { // استخدام jQuery بدلاً من $ مباشرة لتجنب التعارض المحتمل وضمان التحميل
        $('div.thumcont a.imgAsBg ').each(function() {
            ...
            // $(this).find('').insertBefore($(this)); // هذا السطر كان فارغًا في الأصل، لذا تم التعليق عليه أو يمكن إزالته
        });
    });
    ...
    jQuery(function($) {
        // تم دمج الاثنين المتشابهين من هذا الكود
        $('article#topics-list-box-img div#box-img a ').each(function() {
            var link = $(this).attr('href');
            var title = $(this).text();
            $(this).prepend('<a href="'+link+'" title="'+title+'"></a>');
            $(this).children('a').load(link +'main#index .message-inner .bbWrapper img:eq(0)', function() {
                // تم تصحيح اسم الكلاس بإزالة النقطة البادئة إذا كان هذا هو المقصود
                // إذا كان ".movie-card-image-ahla img" هو محدد CSS، فالطريقة ستكون مختلفة
                $(this).children('img').addClass('movie-card-image-ahla-img'); // افترضت أنك تقصد إضافة كلاس
            });
            // $(this).find('').insertBefore($(this));
        });
    });
    ...
    jQuery(function($) {
        $('form#ucp .block .block-content label a ').each(function() {
            ...
            $(this).children('a').load(link +'span#quickreply-img img:eq(0)', function() {
                // تم تصحيح اسم الكلاس بإزالة النقطة البادئة
                $(this).children('img').addClass('contentRow-figure-js-XFUniqueId3-img'); // افترضت أنك تقصد إضافة كلاس
            });
            // $(this).find('').insertBefore($(this));
        });
    });
} else {
    console.error("jQuery is not loaded. Code from 'صور المجله.txt' will not run.");
}
// ================================================================================================
...
// ================================================================================================
// ابدأ: محتوى من ملف "كود تغيير الالوان و الخطوط للمنتدي.txt" (يعتمد على jQuery)
// ================================================================================================
if (typeof jQuery !== 'undefined') {
    jQuery(document).ready(function($) { // Use jQuery alias for safety
        ...
        function applySavedSettings() {
            ...
            updateToggleButtonText('#dark-mode-toggle', darkMode); // استخدام الدالة المحدثة

            const showSidebar = localStorage.getItem('showSidebar') !== 'false';
            updateSidebar(showSidebar);
            updateToggleButtonText('#sidebar-toggle', showSidebar); // استخدام الدالة المحدثة
        ...
        function updateThemeColor(color) {
            ...
            if(styleTrigger.length) styleTrigger.css('background-color', color); // فحص وجود العنصر
        }
        ...
        function updateSidebar(show) {
            ...
            if ($sidebarWrapper.length) { // فحص وجود العنصر
                if (show) {
                    $sidebarWrapper.show();
                } else {
                    $sidebarWrapper.hide();
        ...
        function updateFontSize(size) {
            ...
            const currentFontSizeEl = $('#current-font-size');
            if (currentFontSizeEl.length) currentFontSizeEl.text(newSize + 'px'); // فحص وجود العنصر
        ...
        // تم تحديث هذه الدالة لتغيير النص بناءً على الحالة، ويمكن تخصيص النص كما تريد
        function updateToggleButtonText(selector, isOn) {
            const button = $(selector);
            if (button.length) { // فحص وجود العنصر
                const state = isOn ? 'on' : 'off';
                button.attr('data-state', state);
                // يمكنك إضافة تغيير للنص هنا إذا كان الزر يحتوي على نص يتغير
                // مثال: button.text(isOn ? 'إيقاف' : 'تشغيل');
            }
        }
        ...
        $('#dark-mode-toggle').on('click', function() {
            ...
            updateToggleButtonText(this, newState);
        });
        ...
        $('#reset-font-settings').on('click', function() {
            ...
            if (fontFamilySelect.length) fontFamilySelect.val(defaultFontValue);
        });
        ...
        $('#reset-all-options').on('click', function() {
            ...
            localStorage.removeItem('fontFamily');
            applySavedSettings();
            if (fontFamilySelect.length) fontFamilySelect.val(fontOptions.find(f => f.name === 'Default').value);
        });
    ...
    console.error("jQuery is not loaded. Code from 'كود تغيير الالوان و الخطوط للمنتدي.txt' will not run.");
}
// ================================================================================================
...
// ================================================================================================
// ابدأ: محتوى من ملف "الصوره الشخصيه بصندوق الرد.txt" (يعتمد على jQuery)
// ================================================================================================
if (typeof jQuery !== 'undefined') {
    jQuery(document).ready(function($){ // استخدام $ الآمن
          $.get('/profile?mode=editprofile&page_profil=avatars', function(data) {
              var link = $('#ucp .block .block-content img', data).attr('src'); // استخدام $ هنا آمن
              var quickReplyImgEl = $('#quickreply-img'); // استخدام $ هنا آمن
              if(quickReplyImgEl.length) { // فحص وجود العنصر
                  if(link){
                    quickReplyImgEl.html('<img src="'+link+'" alt="User Avatar">'); // إضافة alt text
                  }else{
                    quickReplyImgEl.html('');
                  }
              }
          });
    });
} else {
    console.error("jQuery is not loaded. Code from 'الصوره الشخصيه بصندوق الرد.txt' will not run.");
}
// ================================================================================================
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    userCells.forEach(cell => {
        ...
        if (userExtras) {
            ...
            // const basePaddingLeftRight = window.getComputedStyle(userExtras).paddingLeft;
            ...
            cell.addEventListener('mouseenter', function() {
                ...
                requestAnimationFrame(() => {
                    const requiredHeight = userExtras.scrollHeight; // حسبت الارتفاع بعد display:block
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if ('IntersectionObserver' in window) {
        ...
        function resetCardAnimation(card) {
            ...
            // card.style.opacity = '0'; // يتم التعامل معه بواسطة الأنيميشن عادة
            // card.style.transform = 'translateX(-100%)'; // يتم التعامل معه بواسطة الأنيميشن عادة
        }
        ...
        cards.forEach(card => {
            card.style.opacity = '1'; // fallback for old browsers
        });
    }
});
// ================================================================================================
